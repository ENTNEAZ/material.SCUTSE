# 软件项目管理:试卷结构
Based on Final-Exam 2008, 2018-2019

Author: @SJoshua 

整体上来看，过去两年卷子的题型相似，但重复率较低。可以猜测它不会考近年考过的概念。优先保证计算题得分。
...
2. Program
   > 项目群指一组相互联系的项目，宜使用协同方法进行管理来获得收益和进行控制，而这种收益和控制在单独管理这些项目时是不易获得的。
3. Triple Constraint
   > 指范围、时间和成本。
4. Deliverable
...
5. Project Management 
   > 项目管理指「在项目活动中运用专门的知识、技能、工具和技术，以满足项目需求」。
6. Project Portfolio Management
   > 项目组合管理指组织将项目以及项目群组合管理，使其作为一个投资组合，从而促成整个企业的成功。
   >
   > 它注重于组织的战略目标，而项目管理解决战术目标。
7. PMI
   > **P**roject **M**anagement **I**nstitute. An international professional society for project managers.
8. PMP
   > **P**roject **M**anagement **P**rofessional. Certification provided by PMI that requires documenting project experience and education, agreeing to follow the PMI code of ethics, and passing a comprehensive exam.
9. PMO
   > **P**roject **M**anagement **O**ffice. An organizational group responsible for coordinating the project management functions throughout an organization.
10. CCB
    > **C**hange **C**ontrol **B**oard is a formal group of people responsible for approving or rejecting changes to a project.
11. SDLC
    > A **S**ystems **D**evelopment **L**ife **C**ycle is a framework for describing the phases of developing information systems.
12. Project Life Cycle
    > A collection of project phases.

### 项目范围管理
1. Scope
   > 范围指开发项目产品所涉及的所有工作和用来开展工作的所有过程。
2. Scope Validation
...
6. Scope Baseline <2018>
   > 批准的项目范围说明书和与之相关的WBS和WBS字典构成了范围基线。

### 项目时间管理
1. CPM
   > **C**ritical **P**ath **M**ethod is a network diagramming technique used to predict total project duration.
2. PERT
   > 计划评审技术是单个活动工期估计高度不确定的情况下用来估计项目工期的技术。
...
### 项目成本管理
1. Cost
   > 成本是为实现一个特定目标而牺牲或放弃的资源。
2. Tangible Costs/Benefits
   > 有形成本/利益可以用简单地用金额衡量。
3. Intangible Costs/Benefits
   > 无形成本/利益难以简单地用金额衡量。
4. Direct Costs
   > 与项目的产品和服务的生产直接相关的成本。
5. Indirect Costs
   > 与项目的产品和服务的生产不直接相关的成本。
6. Sunk Cost
   > 过去已花掉的钱。
7. Reserves
   > 储备金 are dollar amounts included in a cost estimate to mitigate cost risk by allowing for future situations that are difficult to predict.
8. Contingency reserves
   > 应急储备金 allow for future situations that may be partially planned for(**known unknowns**) and are included in the project cost baseline.
9. Management reserves
   > 预备方案 allow for future situations that are unpredictable(**unknown unknowns**).
10. Cost Budgeting
    > 成本预算涉及将项目成本估算随时间分配给个体材料资源或单个工作项。
11. Cost Baseline
    > 成本基线是分时段的预算，项目经理用它来衡量和监控成本性能。

### 项目质量管理
1. Quality 
   > 质量是反映实体满足明确和隐含需求的能力特性总和。
2. Quality Assurance
   > 质量保证(QA) includes all of the activities related to satisfying the relevant quality standards for a project.Another goal of QA is continuous quality improvement.
3. Pareto Diagram
   > 帕累托分析，又称为80-20法则，表示80%的问题经常是由于20%的原因引起的。

### 项目人力资源管理
1. OBS
   > 组织分解结构(**O**rganizational **B**reakdown **S**tructure)是一种特殊的组织结构显示每个组织单元负责哪项工作。
2. RAM
   > 职责分配矩阵(**R**esponsibility **A**ssignment **M**atrix)将WBS中描述的工作与OBS中负责实施的人员匹配。
...
* 项目范围管理
  ...
  > 6. [M]控制范围
* 项目时间管理 <2019>
  > 包括项目所需总时间的估算，编写一个可以接受的项目日程进度计划，并确保项目按时完工。
  > 1. [P]计划进度管理
  > 2. [P]定义活动
  > 3. [P]排序活动
  > 4. [P]估算活动资源
...
* 项目人力资源管理
  ...
  > 4. [M]管理项目团队
* 项目沟通管理
  > 确保项目信息能够及时并且适当地生成、收集、发送、存储和部署。
  > 1. [P]计划沟通管理
  > 2. [E]管理沟通
  > 3. [M]控制沟通
* 项目风险管理
  > 包括对项目相关的风险进行辨析、分析和应对。
  > 1. [P]计划风险管理
  > 2. [P]识别风险
  > 3. [P]实施定向风险分析
  > 4. [P]实施定量风险分析
  ...
  > 6. [M]控制风险
* 项目采购管理
  > 包括为一个项目从外部组织获得商品或服务所需要的过程。
  > 1. [P]计划采购管理
  > 2. [E]实施采购
  > 3. [M]控制采购
  > 4. [C]采购收尾
* 项目干系人管理
  > 识别项目中的所有人员和组织、分析干系人的期望，并在项目整个生命周期的项目决策里有效地吸引项目干系人的参与。
  > 1. [I]干系人识别
  > 2. [P]干系人管理计划
  > 3. [E]参与干系人管理
  > 4. [M]参与干系人控制
* 项目综合管理
  > 涉及在整个项目生命周期中协调所有其他项目管理的知识领域。这种综合确保项目所有的组成要素能在正确的时间整合在一起成功地完成项目。
  > 1. [I]制定项目章程，是指与干系人共同工作，制定一个正式批准项目的文件，即章程。
  > 2. [P]开发项目管理计划，指协调所有的计划努力，制定一个连贯一致的文件——项目管理计划。
  > 3. [E]指挥并管理项目执行，通过执行项目管理计划中的有关活动，来执行项目管理计划。
  > 4. [M]监控项目工作，检查项目活力是否符合项目绩效目标。
